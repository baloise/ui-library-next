import { Component, Host, h, Element, State, Listen, Event, EventEmitter, Method } from '@stencil/core'
import { StepOptions } from '../bal-step/bal-step'
        ...
        <div class={['tabs is-fullwidth'].join(' ')}>
          <ul>
            {this.stepOptions.map((step, index) => (
              <li class={[step.active ? 'is-active' : '', step.disabled ? 'is-disabled' : ''].join(' ')}>
                ...
                <span class="bubble" style={!step.hasBubble && { display: 'none' }}></span>
              </li>
            ))}
          </ul>
        </div>
